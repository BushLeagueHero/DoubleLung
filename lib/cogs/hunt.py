...
class Hunt(Cog):
    ...
    def __get_speciesid(self,ctx,species,group):
        logger.debug(f"getting the id for {species}.")
        for i in range(0,len(data["species"])):
            if data["species"][i]["speciesid"] == species:
                speciesid = data["species"][i]
        
        self.__get_embeds(ctx,species,speciesid,group)

    def __get_embeds(self,ctx,species,speciesid,group):
        logger.debug(f"loading hunt stats for {species}, showing {group}")
        embeds = []
        statkeys = self.__determine_stat_keys(group)
        logger.debug(f"populating embeds for the following stats: {statkeys}")
        for stat in statkeys:
            embed = Embed(title=data["species"][speciesid]["species"],color=0xFF0000)
            embed.set_author(name="DoubleLung Bot")
            embed.set_thumbnail(url=ctx.message.guild.icon_url)
            
            self.__add_stat_to_embed(embed,species,speciesid,stat)
    ...
    def __add_stat_to_embed(self,embed,species,speciesid,stat):
        logger.debug(f"populating data for {stat} using {species}")
        for statkey in data["species"][speciesid]:
    ...
    def __determine_stat_keys(self,group):
        logger.debug(f"determining stat keys for stat {group}")
        keys = []
        if user_stat == "all":
    ...
    async def get_mallard(self,ctx,*,species,group):
        embeds = self.__get_speciesid(ctx,species,group)
        for embed in embeds:
